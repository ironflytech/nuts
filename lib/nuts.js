...
function Nuts (opts) {
  ...
  if (_.includes(['s3','amazons3','amazon'], this.opts.backend.toLowerCase())) {
    this.opts.backend = 's3'
  }
  ...
  // Create backend
  var Backend = BACKENDS(this.opts.backend)
  this.backend = new Backend(this, this.opts)
  this.versions = new Versions(this.backend, this.opts.maxVersion)

  // Bind routes
  this.router.use(useragent.express())

  this.router.get('/', this.onDownloadLatest)
  this.router.get('/download/channel/:channel/:platform?', this.onDownloadSquirrel)
  this.router.get('/download/version/:tag/:platform?', this.onDownloadSquirrel)
  this.router.get('/download/:tag/:filename', this.onDownloadSquirrel)
  this.router.get('/download/:platform?', this.onDownloadLatest)
  ...
  this.router.get('/update/:platform/:version/RELEASES', this.onUpdateWinSquirrel)
  this.router.get('/update/channel/:channel/:platform/:version/RELEASES', this.onUpdateWinSquirrel)
  this.router.get('/update/nsis/:platform/:version/latest.yml', this.onUpdateWinNSIS)
  this.router.get('/update/nsis/:platform/:version/:filename', this.onDownloadNSIS)
...
Nuts.prototype.onDownloadNSIS = function (req, res, next) {
  this.onDownload('nsis', req, res, next)
}
// Handler for download routes
Nuts.prototype.onDownloadSquirrel = function (req, res, next) {
  this.onDownload('squirrel', req, res, next)
}
// Handler for download routes
Nuts.prototype.onDownloadLatest = function (req, res, next) {
  this.onDownload('unknown', req, res, next)
}
// Handler for download routes
Nuts.prototype.onDownload = function (type, req, res, next) {
  var that = this
  ...
  if (!filename) {
    ...
    if (!platform) {
      ...
      if (req.useragent.isLinux64) {
        platform = platforms.LINUX_64
      }
    }

    if (!platform) {
      return next(new Error('No platform specified and impossible to detect one'))
    }
  } else {
    platform = null
  }
  if (type === 'nsis' && filename) {
    //extract the version out of the filename
    // example filenames: tms-client-setup-7.5.10.exe matches: -7.5.10.exe with group1: 7.5.10
    // example filenames: tms-client-setup-7.5.10-beta.exe matches: -7.5.10-beta.exe with group1: 7.5.10-beta
    var versionMatch = filename.match(/-([0-9]{1,}\.[0-9]{1,}\.[0-9]{1,}-{0,1}.{0,})\.exe/)
    if (versionMatch) {
      tag = versionMatch[1]
    }
  }
  ...
  // Fallback to any channels if no version found on stable one
    ...
    .then(function (version) {
      ...
      if (!asset) {
        ...
        var errText = 'No download available for platform ' + platform + ' for version ' + version.tag +
          ' (' + (channel || 'beta') + ')'
        if (filename) { errText += ' for file: ' + filename }
  ...
  Q()
    .then(function () {
      if (!req.query.version) {
        throw new Error('Requires "version" parameter')
      }
...
Nuts.prototype.onUpdate = function (req, res, next) {
  ...
  Q()
    ...
    .then(function (versions) {
      ...
      res.status(200).send({
        'url': urljoin(fullUrl, gitFilePath, '/download/version/' +
          latest.tag +
          '/' +
          platform +
          '?filetype=' +
...
// Update Windows (NSIS/Squirrel.Windows)
// Auto-updates: Squirrel.Windows: serve RELEASES/latest.yml from latest version
// Currently, it will only serve a full.nupkg of the latest release with a normalized filename (for pre-release)
Nuts.prototype.onUpdateWin = function (type, req, res, next) {
  var that = this
  ...
  that.init()
    .then(function () {
      platform = platforms.detect(platform)
      // console.log('X', platform, channel, tag)
      return that.versions.filter({
        tag: '>=' + tag,
        platform: platform,
        channel: channel
    ...
    .then(function (versions) {
      ...
      // File exists
      var filename = type === 'squirrel' ? 'RELEASES' : 'latest.yml'
      var asset = _.find(latest.platforms, {
        filename: filename
      })
      // console.log('AAA', asset, latest.platforms)
      if (!asset) {
        throw new Error('File not found')
      }

      if (type === 'nsis') {
        return that.serveAsset(req, res, latest, asset).fail(next)
      }
...
// Update Windows (NSIS)
// Auto-updates: NSIS: serve latest.yml from latest version
// Currently, it will only serve a full.nupkg of the latest release with a normalized filename (for pre-release)
Nuts.prototype.onUpdateWinNSIS = function (req, res, next) {
  this.onUpdateWin('nsis', req, res, next)
}

// Update Windows (Squirrel.Windows)
// Auto-updates: Squirrel.Windows: serve RELEASES from latest version
// Currently, it will only serve a full.nupkg of the latest release with a normalized filename (for pre-release)
Nuts.prototype.onUpdateWinSquirrel = function (req, res, next) {
  this.onUpdateWin('squirrel', req, res, next)
}
